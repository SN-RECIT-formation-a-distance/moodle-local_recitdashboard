...
import { Card, ButtonGroup, ButtonToolbar, Button, OverlayTrigger, Tooltip, Badge} from 'react-bootstrap';
import {faSync, faInfo, faSearchPlus, faFileSignature, faUserGraduate} from '@fortawesome/free-solid-svg-icons';
        ...
        let main = 
            <div style={{border: "1px solid #efefef"}}>
                ...
                {this.props.data.students.map((item, index) => {
                    let row = <div key={index} style={{display: "grid", gridTemplateColumns: "60% auto auto auto", marginBottom: ".2rem", padding: ".5rem"}}>
                                ...
                                <div><Button size="sm" variant={(item.newStudent === 0 ? 'outline-success' : 'outline-danger')} title="Nouveau élève"><FontAwesomeIcon icon={faUserGraduate}/></Button></div>
                                <div><Button size="sm" variant={(item.nbLateActivities === 0 ? 'outline-success' : 'outline-danger')} title="Activités en retard"><Badge variant="light">{item.nbLateActivities}</Badge></Button></div>
                            </div>;
                    return row;
                })}
            </div>;
